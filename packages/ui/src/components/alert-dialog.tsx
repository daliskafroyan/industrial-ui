    ...
    <AlertDialogPortal {...portalProps}>
      ...
      >
        <AlertDialogPrimitive.Popup
          className={cn(
            "relative row-start-2 flex max-h-full min-h-0 w-full min-w-0 max-w-lg origin-center flex-col rounded-none border bg-popover not-dark:bg-clip-padding text-popover-foreground opacity-[calc(1-var(--nested-dialogs))] shadow-lg/5 transition-[scale,opacity,translate] duration-200 ease-in-out will-change-transform before:pointer-events-none before:absolute before:inset-0 before:rounded-none before:shadow-[0_1px_--theme(--color-black/4%)] data-ending-style:opacity-0 data-starting-style:opacity-0 sm:scale-[calc(1-0.1*var(--nested-dialogs))] sm:data-ending-style:scale-98 sm:data-starting-style:scale-98 dark:before:shadow-[0_-1px_--theme(--color-white/6%)]",
            bottomStickOnMobile &&
...
export function AlertDialogFooter({
  ...
  return (
    <div
      className={cn(
        "flex flex-col-reverse [&>*]:min-w-0 [&>*]:w-full [&>*]:justify-center [&>*]:text-lg [&>*]:h-11 [&>*]:px-[calc(--spacing(4)-1px)] sm:flex-row sm:[&>*]:flex-1 sm:[&>*]:text-base sm:[&>*]:h-10",
